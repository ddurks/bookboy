...
const K = { BODY: 0, TITLE: 1, SUBTITLE: 2, BYLINE: 3, PART: 4, PARTSUB: 5,
            DISPLAY: 6 };
const CLASS_KIND = { fmh: K.TITLE, cst: K.SUBTITLE, ctag: K.BYLINE,
                     pt: K.PART, fmsh: K.PARTSUB, center: K.DISPLAY,
                     ct: K.TITLE };
...
function extractParas(doc) {
  const paras = [];
  let cur = null, italic = 0;

  const flush = (keep) => {
    if (!cur) return;
    if (cur.runs.some((r) => r.text.trim() !== ''))
      paras.push(cur);
...
async function parseEpub(file) {
  ...
  for (const ref of opf.querySelectorAll('spine > itemref')) {
    ...
    if (!it) continue;
    const isDoc = /html|oeb1-document/.test(it.type) ||
                  /\.(x?html?|xml|xhtm)$/i.test(it.href);
    if (!isDoc) continue;
    const bytes = await zip.read(joinPath(base, it.href));
    if (!bytes) continue;
    const doc = parseXml(bytes, 'application/xhtml+xml') ||
  ...
  let coverRGB = null, coverW = 0, coverH = 0;
  if (coverHref) {
    try {
      ...
      for (let i = 0, j = 0; i < img.length; i += 4, j += 3) {
        coverRGB[j] = img[i]; coverRGB[j + 1] = img[i + 1];
        coverRGB[j + 2] = img[i + 2];
      }
      coverW = bmp.width; coverH = bmp.height;
    } catch (e) { /* no cover, fine */ }
  }
  const nchars = chapters.flat().reduce(
    (a, p) => a + p.runs.reduce((b, r) => b + r.text.length, 0), 0);
...
async function initCore() {
  ...
  for (const f of ['XCharter-Roman.otf', 'XCharter-Italic.otf',
                   'XCharter-Bold.otf'])
    core.FS.writeFile('/fonts/' + f,
      new Uint8Array(await (await fetch('assets/' + f)).arrayBuffer()));
  core.FS.writeFile('/hyph_en_US.dic',
  ...
  for (const paras of b.chapters) {
    ...
    for (const p of paras) {
      ...
      for (const r of p.runs) {
        ...
        cc('bw_para_run', null, ['number', 'number', 'number'],
...
function totalBytes() {
  const per = books.reduce((a, b) =>
    a + (b.blobSize ? ((b.blobSize + 3) & ~3) : 0) +
    ($('covers').checked && b.coverRGB ? COVER_BYTES + 10000 : 0) + 96, 0);
...
function renderPreview() {
  ...
  if (!b || !b.npages || b.npages <= 0) { wrap.style.display = 'none'; return; }
  wrap.style.display = '';
  const cv = $('gba');
  const ctx = cv.getContext('2d');
  const ptr = core._malloc(240 * 160 * 4);
  const ok = cc('bw_render_page', 'number', ['number', 'number', 'number'],
...
async function fetchAssets() {
  ...
  assetsCache = { stub: new Uint8Array(stub), title: new Uint8Array(title),
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  $('prev').onclick = () => { if (curPage > 0) { curPage--; renderPreview(); } };
  $('next').onclick = () => {
    if (books[selected] && curPage + 1 < books[selected].npages)
      { curPage++; renderPreview(); }
  };
